...
def annotate(sql: str, dialect: str = "tsql", terminate_all: bool = False,
    ...
    for idx, t in enumerate(toks):
        ...
        if is_starter and depth == 0:
            ...
            # SELECT/EXEC, CTE WITH's main query) still advances the lead so the
            # NEXT statement is no longer treated as part of it - EXCEPT a DDL IF
            # clause, an UPDATE/MERGE SET clause, and a non-CTE WITH (table hint /
            # proc option), which are clauses of the current statement, not leads.
            if (not ddl_if
                    and not (current_lead in {"UPDATE", "MERGE"} and txt == "SET")
                    and not with_hint and not with_option):
                current_lead = txt
            seen_first = True

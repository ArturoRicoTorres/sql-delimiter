...
procedural code (DECLARE/SET/SELECT/EXEC/RETURN/IF/WHILE/TRY-CATCH). In the
default mode every detected boundary is delimited (a best-effort guess - review
the output). In conservative mode (conservative=True) high-confidence statement
starters are always delimited, while low-confidence ones (which double as
sub-clauses) are delimited only when corroborated, and uncorroborated ones are
flagged for review instead of being cut. See annotate() for details.
"""
...
def annotate(sql: str, dialect: str = "tsql", terminate_all: bool = False,
    ...
    for idx, t in enumerate(toks):
        ...
        if is_starter and depth == 0:
            ...
            # WITH that is NOT a CTE: a table hint - WITH (TABLOCK/NOLOCK ...) -
            # or a procedure/function option - WITH RECOMPILE / ENCRYPTION. Both
            # are clauses of the current statement, not a new lead.
            with_hint = txt == "WITH" and nxt == "("
            with_option = txt == "WITH" and current_lead in {"CREATE", "ALTER"}
            suppress = False
            if not seen_first:
                suppress = True
            elif expect_body:
            ...
            # SELECT/EXEC, CTE WITH's main query, a proc-option WITH) still
            # advances the lead so the NEXT token is read in the new context -
            # EXCEPT a DDL IF clause, an UPDATE/MERGE SET clause, and a table-hint
            # WITH ( ... ), which are clauses of the CURRENT statement. Note a
            # proc-option WITH (WITH RECOMPILE) MUST advance to WITH so that a CTE
            # main query (CREATE PROC ... AS WITH cte AS (...) INSERT ...) is still
            # recognised and not cut from its WITH.
            if (not ddl_if
